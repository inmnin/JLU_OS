...
public class Lamport_Output extends OutPut{
    public void taken_number_output(int id,int Number){
        right_text.append("进程"+id+"取到号码："+Number+"\n");
        mid_text.setCaretPosition(mid_text.getDocument().getLength());
    }
    public void is_taking_number_output(int id){
        mid_text.append("进程"+id+"被拒绝，已经有进程在面包门口取号\n");
        mid_text.setCaretPosition(mid_text.getDocument().getLength());
    }

    public void waiting_output(int id){
        mid_text.append("进程"+id+"想进入面包店却发现已经有人了\n");
        mid_text.setCaretPosition(mid_text.getDocument().getLength());
    }
    public void ready_output(int id,int Number){
        mid_text.append("\n"+"--进程"+id+"正在访问临界区--\n"+"其面包店号码为："+Number+"\n\n");
        mid_text.setCaretPosition(mid_text.getDocument().getLength());
    }
